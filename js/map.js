    ...
    if (this.motor === 'google') {
      ...
      if (!this._marker) {
        this._marker = new google.maps.Marker({
          map: this._map, position: { lat, lng },
          icon: { path: google.maps.SymbolPath.CIRCLE, scale: 7, fillColor: '#ff5c2a', fillOpacity: 1, strokeColor: '#fff', strokeWeight: 2 }
        });
      } else this._marker.setPosition({ lat, lng });
    } else {
      this._line.addLatLng([lat, lng]);
      if (!this._marker) {
        this._marker = L.circleMarker([lat, lng], { radius: 7, color: '#fff', fillColor: '#ff5c2a', fillOpacity: 1 }).addTo(this._map);
      } else this._marker.setLatLng([lat, lng]);
    }
    this.center(lat, lng, 15);
  },
